...
opts.expDir = fullfile('data', 'leap-binary-full-large');
opts.imdbPath = fullfile(opts.expDir, 'imdb.mat');
opts.train.batchSize = 20 ;
opts.train.numEpochs = 2000 ;  
opts.train.continue = true ;  % can continue training after stopping
opts.train.useGpu = false ;
opts.train.learningRate = [0.1*ones(1, 250) 0.01*ones(1, 250) 0.001*ones(1, 500) 0.0001*ones(1,500)  0.00001*ones(1,500)] ;
...
net.layers{end+1} = struct('type', 'conv', ...
                           'filters', f*randn(3,3,1,30, 'single'), ...
...
net.layers{end+1} = struct('type', 'pool', ...
                           ...
                           'pad', 0) ;
net.layers{end+1} = struct('type', 'conv', ...
                           'filters', f*randn(5,5,30,60, 'single'),...
...
net.layers{end+1} = struct('type', 'pool', ...
                           ...
                           'pad', 0) ;
net.layers{end+1} = struct('type', 'conv', ...
                           'filters', f*randn(5,4,60,100, 'single'),...
...
net.layers{end+1} = struct('type', 'pool', ...
                           ...
                           'pad', 0) ;
net.layers{end+1} = struct('type', 'conv', ...
                           'filters', f*randn(9,9,100,200, 'single'),...
...
net.layers{end+1} = struct('type', 'pool', ...
                           ...
                           'pad', 0) ;
net.layers{end+1} = struct('type', 'conv', ...
                           'filters', f*randn(10,4,200,300, 'single'),...
                           'biases', zeros(1,300,'single'), ...
                           'stride', 1, ...
                           'pad', 0) ;
net.layers{end+1} = struct('type', 'relu') ;
net.layers{end+1} = struct('type', 'conv', ...
                           'filters', f*randn(1,1,300,100, 'single'),...
                           'biases', zeros(1,100,'single'), ...
                           'stride', 1, ...
                           'pad', 0) ;
net.layers{end+1} = struct('type', 'relu') ;
net.layers{end+1} = struct('type', 'conv', ...
                           'filters', f*randn(1,1,100,opts.train.outputClasses, 'single'),...
