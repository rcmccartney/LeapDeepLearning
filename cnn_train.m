...
% after training get full confusion matrix
fprintf('Printing confusion matrix..\n') ;
conf = zeros(opts.outputClasses , opts.outputClasses) ;
val = opts.val ;    
...
for t=1:opts.batchSize:numel(val)
    ...
    % predictions are the sorted indices of the output nodes
    [~,predictions] = sort(predictions, 3, 'descend') ;
    for i=1:size(labels, 2),
      conf(predictions(:,:,1,i), labels(i))=conf(predictions(:,:,1,i), labels(i)) + 1 ;
      if predictions(:,:,1,i) ~= labels(i),
          fprintf('Instance %d in val %d wrong: actual %d predicted %d\n', i, t, ...
              labels(i), predictions(:,:,1,i))
      end;
    end;
end;

for i=1:opts.outputClasses,
    for j=1:opts.outputClasses,
        fprintf('%5d ', conf(i, j));
    end;
    fprintf('\n');
end;
