function [net, info] = cnn_leap_2(varargin)
...
opts.expDir = fullfile('data','leap-2') ;
opts.imdbPath = fullfile(opts.expDir, 'imdb.mat');
opts.train.batchSize = 100 ;
opts.train.numEpochs = 1400 ;
...
linesToSkip = 2;
dim = 200;
sampleSize = 28; % using same as MNIST for now...
...
for i=1:length(directoryNames),
    ...
    for j=1:length(folderNames),
        ...
        for k=1:length(fileNames),
            image = ImageMaker(fullfile(top, directoryNames{i}, ...
                        folderNames{j}, fileNames{k}), ...
                        dim, sampleSize, linesToSkip);
            % imshow(image)   
            if ~isempty(image),
                images(:,:,index) = image;
...
trainsize = int64(0.7*size(output,2));
...
data = single(reshape(images,sampleSize,sampleSize*3,1,[]));
